...
summary(GetModelRegSubset( 16,reg.dev,train.data,reg.formula,nvmax,nbest))

lm.model <- GetModelRegSubset( 16,reg.dev,train.data,reg.formula,nvmax,nbest)
...
xgb.model <- xgboost::xgb.train(param=param, data = xgb.train, nthread = 4, nround = 750,
                      watchlist=watchlist,## eval_metric = "rmse",
                      verbose=1, print.every.n = 250, early.stop.round= 101,maximize = TRUE)

print(paste("best: index:",xgb.model$bestInd, "train-gini:",xgb.model$bestScore))
...
write.csv(data.frame(Id=submit.raw$Id, Hazard=submit.pred),"R_xgb_dev20150827_1.csv",row.names=F, quote=FALSE)
